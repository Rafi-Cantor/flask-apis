...
def join_chat(data):
    ...
    try:
        ...
        emit("error", {
            "error_message": f"Chat {chat_id} not found. "
        })
        return
    join_room(chat_id)
...
def chats():
    if request.method == "GET":
        ...
        user_chats = user_chat_mapping.UserChatMapping.get_chats_for_user(user_id=current_user.user_id)
...
@views.multi_auth.login_required
def get_chat_messages(chat_id: uuid.UUID):
    current_user = views.multi_auth.current_user()

    try:
        user_chat_mapping.UserChatMapping.check_exists(user_id=current_user, chat_id=chat_id)
    except user_chat_mapping.UserChatMappingDoesntExist:
        return jsonify({"error_message": f"Chat {chat_id} not found. "}), 404
